...
namespace Tarefas.Salvar.DataBase.Repositorios
{
    public class TarefaRepositorio : ITarefaRepositorio
    {
        ...
        {
            return new MySqlConnection ( "Server=localhost;Port=3306;Database=dbtarefa;Uid=sa;Pwd=123456");
            ...
            try
            {
                if (tarefa != null && tarefa.Id != 0)
                {
                    using (MySqlConnection connection = GetSqlConnection())
                    {
                        ...
                        var query = "UPDATE tarefas  SET Descricao = @Descricao, Data = @Data, Status = @Status WHERE Id = @Id";
